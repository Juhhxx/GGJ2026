...
public class EnemyPatrolMovement : MonoBehaviour, IMovementType
{
    ...
    public enum Space { TwoD, ThreeD }
    [SerializeField] private Space _space = Space.TwoD;
    ...
    private void CreatePath()
    {
        ...
        Vector3 defaultAxis = _space == Space.TwoD ? Vector3.up : Vector3.forward;
        // Create Default Square Path
        path.CreateWaypoint(0, pos);
        path.CreateWaypoint(1, pos += (Vector3.right * 2.5f));
        path.CreateWaypoint(2, pos += (defaultAxis * 2.5f));
    ...
    public float Speed => _space == Space.TwoD ?
    _rb2D.linearVelocity.magnitude : _rb.linearVelocity.magnitude;
    ...
    public void Move()
    {
        if (_space == Space.TwoD) _rb2D.linearVelocity = _motion;
        else _rb.linearVelocity = _motion;
    }
